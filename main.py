...
STATUS_META = {
    ...
    STATUS_THINKING:  {"text": "Connecting…",   "sub": "Linking to Gemini API",   "color": [0.2,  0.6,  1.0,  1]},
    STATUS_SPEAKING:  {"text": "Speaking",      "sub": "MPro is responding",      "color": [0.0,  1.0,  0.7,  1]},
    STATUS_ERROR:     {"text": "Error",         "sub": "Check connection",        "color": [1.0,  0.3,  0.3,  1]},
}

# ── Pulse ring widget drawn on canvas ──────────────────────────────────────
        ...
        with self.canvas:
            for i in range(n):
                ...
                wave = (
                    ...
                    math.sin(t * self._speeds[i] * 1.7 + self._phases[i] * 0.7) * 0.2 +
                    0.45
                )
                ...
                x = i * (bar_w + gap)
                y = (h - bar_h) / 2
                dist_center = abs(i - n / 2) / (n / 2)
                alpha = a * (1.0 - dist_center * 0.35)
                Color(r, g, b, alpha)
...
<MPROScreen>:
    ...
    canvas:
        Color:
            rgba: 0.12, 0.55, 0.85, 0.04
        Line:
            points: [0, dp(40)*i for i in range(1,60) for _ in (0,1)] if False else []
